...
//
// BUILD-VERIFIED ONLY: this compiles and links (hook/build.bat), but its runtime
// is validated against a live D3D11 app, not in CI. Per the project doctrine we
// do not claim it works from compilation alone.
//
...
// Canonical ID3D11DeviceContext vtable slot indices (interface is frozen).
enum {
...
// Stage codes match the C ABI: 0=Vs 1=Ps 2=Cs. Kind codes: 0=Srv 1=Rtv 2=Dsv 4=Uav.
...
static void STDMETHODCALLTYPE hkOM(ID3D11DeviceContext* c, UINT n, ID3D11RenderTargetView* const* rtvs, ID3D11DepthStencilView* dsv) {
    if (g_fs) {
        ...
        // DSV read-only detection (GetDesc flags) is a TODO; treated as a write.
        if (dsv) ft_register_view(g_fs, (uint64_t)dsv, resource_of((ID3D11View*)dsv), 2);
...
static bool install() {
    // A throwaway device just to read the shared immediate-context vtable.
    ...
    return true;
    // GS/HS/DS SRV stages and OMSetRenderTargetsAndUnorderedAccessViews are easy
    // additional slots (31/59/63, 34) following the same pattern.
}
